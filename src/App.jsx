...
import { ThemeProvider } from './contexts/ThemeContext.jsx' // <--- IMPORTADO
...
function Navigation() {
  ...
  // Lógica de cores atualizada para Dark Mode
  const getIconClass = (path) => {
    const isActive = location.pathname === path
    // Light: Azul escuro / Dark: Azul claro (para contraste)
    return isActive 
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-900 border-t border-gray-200 dark:border-slate-800 py-2 px-1 flex justify-around items-center z-50 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] select-none transition-colors duration-300">
      ...
      <Link to="/foco" className="flex flex-col items-center -mt-6">
        ...
        <span className="text-[9px] font-bold uppercase tracking-wide text-[#0047AB] dark:text-blue-400 mt-1">Foco</span>
      </Link>
